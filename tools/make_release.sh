...
# writable at flash offset 0x0.  Output: dist/the-game-box.bin
set -euo pipefail
cd "$(dirname "$0")/.."
...
"$HOME/.platformio/penv/bin/python" -m esptool --chip esp32s3 merge_bin \
  ...
  -o dist/the-game-box.bin \
  0x0     "$BUILD/bootloader.bin" \
  0x8000  "$BUILD/partitions.bin" \
  0xe000  "$BOOT_APP0" \
  0x10000 "$BUILD/firmware.bin"

echo ">> dist/the-game-box.bin"
ls -l dist/the-game-box.bin
echo ">> flash with:  esptool.py --chip esp32s3 -p <port> write_flash 0x0 dist/the-game-box.bin"
